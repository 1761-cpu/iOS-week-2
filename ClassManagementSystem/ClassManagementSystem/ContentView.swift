import SwiftUI

struct ContentView: View {
    @State private var students: [Student] = [
        Student(studentId: "S001", name: "An", gpa: 8.5),
        Student(studentId: "S002", name: "Binh", gpa: 7.8),
        Student(studentId: "S003", name: "Chi", gpa: 9.2),
        Student(studentId: "S004", name: "Dung", gpa: 6.5),
        Student(studentId: "S005", name: "Em", gpa: 8.0)
    ]
    
    @State private var searchText: String = ""
    @State private var showAddView: Bool = false
    @State private var showGoodOnly: Bool = false   // Lọc GPA >= 8.0
    @State private var showTopOnly: Bool = false    // Chỉ hiện Top GPA
    
    // 0 = không sort, 1 = tăng dần, 2 = giảm dần
    @State private var sortMode: Int = 0  // Sắp xếp theo GPA
    
    // MARK: - Filtered & Sorted Students
    var displayedStudents: [Student] {
        var result = students
        
        // 1. Lọc GPA >= 8.0
        if showGoodOnly {
            result = result.filter { $0.gpa >= 8.0 }
        }
        
        // 2. Chỉ hiện Top GPA
        if showTopOnly, let top = result.max(by: { $0.gpa < $1.gpa }) {
            result = [top]
        }
        
        // 3. Tìm kiếm theo tên hoặc ID
        if !searchText.isEmpty {
            result = result.filter {
                $0.name.lowercased().contains(searchText.lowercased()) ||
                $0.studentId.lowercased().contains(searchText.lowercased())
            }
        }
        
        // 4. Sắp xếp theo GPA (2 chế độ)
        if sortMode == 1 {
            result = result.sorted { $0.gpa < $1.gpa }   // Tăng dần
        } else if sortMode == 2 {
            result = result.sorted { $0.gpa > $1.gpa }   // Giảm dần
        }
        
        return result
    }
    
    var body: some View {
        NavigationStack {
            VStack(spacing: 12) {
                
                // ===== HEADER =====
                VStack(spacing: 8) {
                    Image(systemName: "person.3.fill")
                        .font(.system(size: 48))
                        .foregroundColor(.blue)
                    
                    Text("STUDENT MANAGER")
                        .font(.system(size: 22, weight: .bold, design: .monospaced))
                    
                    Text("A better class, a brighter tomorrow")
                        .font(.system(size: 14, weight: .semibold))
                        .italic()
                        .foregroundColor(.gray)
                }
                .padding(.vertical, 16)
                .frame(maxWidth: .infinity)
                .background(Color.white.opacity(0.65))
                .clipShape(RoundedRectangle(cornerRadius: 20))
                .padding(.horizontal)
                .padding(.top, 8)
                
                // ===== SEARCH FIELD =====
                TextField("Search by name or ID...", text: $searchText)
                    .textFieldStyle(RoundedBorderTextFieldStyle())
                    .padding(.horizontal)
                
                // ===== QUICK ACTION BUTTONS =====
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 8) {
                        // Top GPA
                        Button(action: {
                            showTopOnly.toggle()
                            if showTopOnly { showGoodOnly = false }
                        }) {
                            Label("Top GPA", systemImage: "crown.fill")
                                .font(.caption)
                                .padding(.horizontal, 12)
                                .padding(.vertical, 8)
                                .background(showTopOnly ? Color.purple : Color.gray.opacity(0.3))
                                .foregroundColor(showTopOnly ? .white : .primary)
                                .cornerRadius(20)
                        }
                        
                        // GPA >= 8.0
                        Button(action: {
                            showGoodOnly.toggle()
                            if showGoodOnly { showTopOnly = false }
                        }) {
                            Label("GPA ≥ 8", systemImage: "star.fill")
                                .font(.caption)
                                .padding(.horizontal, 12)
                                .padding(.vertical, 8)
                                .background(showGoodOnly ? Color.yellow : Color.gray.opacity(0.3))
                                .foregroundColor(showGoodOnly ? .white : .primary)
                                .cornerRadius(20)
                        }
                        
                        // Sort by GPA
                        Button(action: {
                            // 0 → 1 → 2 → 0 (vòng lặp)
                            sortMode = (sortMode + 1) % 3
                        }) {
                            Label(
                                sortMode == 0 ? "Sort GPA" :
                                sortMode == 1 ? "Low → High" : "High → Low",
                                systemImage: sortMode == 1 ? "arrow.up" :
                                             sortMode == 2 ? "arrow.down" : "arrow.up.arrow.down"
                            )
                            .font(.caption)
                            .padding(.horizontal, 12)
                            .padding(.vertical, 8)
                            .background(sortMode == 0 ? Color.gray.opacity(0.3) : Color.indigo)
                            .foregroundColor(sortMode == 0 ? .primary : .white)
                            .cornerRadius(20)
                        }
                        
                        // Reset Filters
                        Button(action: {
                            showTopOnly = false
                            showGoodOnly = false
                            sortMode = 0
                            searchText = ""
                        }) {
                            Label("Reset", systemImage: "arrow.counterclockwise")
                                .font(.caption)
                                .padding(.horizontal, 12)
                                .padding(.vertical, 8)
                                .background(Color.red.opacity(0.8))
                                .foregroundColor(.white)
                                .cornerRadius(20)
                        }
                    }
                    .padding(.horizontal)
                }
                
                // ===== DANH SÁCH SINH VIÊN =====
                List {
                    ForEach(displayedStudents) { student in
                        NavigationLink {
                            EditStudentView(students: $students, student: student)
                        } label: {
                            HStack {
                                Image(systemName: "person.circle.fill")
                                    .font(.system(size: 30))
                                    .foregroundColor(.blue)
                                
                                VStack(alignment: .leading) {
                                    Text(student.name)
                                        .font(.headline)
                                    Text("ID: \(student.studentId)")
                                        .font(.caption)
                                        .foregroundColor(.gray)
                                }
                                
                                Spacer()
                                
                                Text("GPA: \(String(format: "%.1f", student.gpa))")
                                    .font(.subheadline)
                                    .foregroundColor(student.gpa >= 8.0 ? .green : .orange)
                            }
                            .padding(.vertical, 4)
                        }
                    }
                    .onDelete { indexSet in
                        // Xóa sinh viên khi vuốt
                        for index in indexSet {
                            let student = displayedStudents[index]
                            students.removeAll { $0.studentId == student.studentId }
                        }
                    }
                }
                .listStyle(.plain)
                
                // ===== NÚT ADD STUDENT =====
                Button(action: {
                    showAddView = true
                }) {
                    HStack {
                        Image(systemName: "plus")
                        Text("Add Student")
                    }
                    .frame(maxWidth: .infinity)
                    .padding()
                    .background(Color.blue)
                    .foregroundColor(.white)
                    .cornerRadius(10)
                }
                .padding(.horizontal)
                
                // ===== TỔNG SỐ SINH VIÊN =====
                Text("Total students: \(students.count)")
                    .font(.footnote)
                    .foregroundColor(.gray)
                    .padding(.bottom, 8)
            }
            .sheet(isPresented: $showAddView) {
                AddStudentView(students: $students)
            }
        }
    }
}

#Preview {
    ContentView()
}
